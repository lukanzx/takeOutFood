...
public class OrdersItemController {
     ...
     * 根据OrderID查询
     */
    @GetMapping("/selectByOrderIdExtra/{id}")
    public Result selectByOrderIdExtra(@PathVariable Integer id) {
        List<OrdersItem> ordersItem = ordersItemService.selectByOrderIdExtra(id);
        return Result.success(ordersItem);
    }
