...
<script>
	export default {
		...
		methods: {
			...
			deleteAll() {
				let xmOrders = uni.getStorageSync('xm-orders') || {}
				this.$request.del('/cart/deleteByBusiness/' + xmOrders.businessId + '/' + this.user.id).then(res => {
					if (res.code === '200') {
						uni.showToast({
							icon: 'success',
							title: '操作成功'
			...
			loadCart() {
				...
				this.$request.get('/cart/selectAll', { userId: this.user.id, businessId: xmOrders.businessId  }).then(res => {
					this.cartList = res.data || []
					this.options[0].info = this.cartList.length
				})
				
				this.$request.get('/cart/calc', { userId: this.user.id, businessId: xmOrders.businessId }).then(res => {
					this.amount = res.data || {}
				})
			},
			addCart(goods) {
				this.$request.post('/cart/add', { goodsId: goods.id, num: 1, businessId: goods.businessId, userId: this.user.id }).then(res => {
					let xmOrders = uni.getStorageSync('xm-orders') || {}
					xmOrders.businessId = goods.businessId
					uni.setStorageSync('xm-orders', xmOrders)
					if (res.code === '200') {
						uni.showToast({
							icon: 'success',
